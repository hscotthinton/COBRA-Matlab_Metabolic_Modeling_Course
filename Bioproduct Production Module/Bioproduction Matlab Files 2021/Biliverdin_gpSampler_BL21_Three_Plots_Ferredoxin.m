% Biliverdin_gpSampler_BL21_Three_Plots_Ferredoxin.m
clear;
...
% model = changeRxnBounds(model,'Ec_biomass_iJO1366_core_53p95M)',0.2,'l');

% Low production model
model_LP = changeRxnBounds(model,'EX_biliverdin(e)',0.01,'l');
sampleStruct_LP = gpSampler(model_LP,6000,[],120);

% Medium production model
model_M = changeRxnBounds(model,'EX_biliverdin(e)',0.1,'l');
sampleStruct_M = gpSampler(model_M,6000,[],120);


% High production model
model_HP = changeRxnBounds(model,'EX_biliverdin(e)',0.8,'l');
sampleStruct_HP = gpSampler(model_HP,6000,[],120);

% Visualize sampling results for a set of reactions. 
rxnList = {'EX_biliverdin(e)','HEMEOX', 'FCLT', 'GLUDy', 'ICDHyr', 'NADTRHD', 'FNADPR','ATPS4rpp','THD2pp','GND','GLNS','NADH16pp','Ec_biomass_iJO1366_core_53p95M', 'EX_glc(e)', 'EX_o2(e)'};
plotSampleHist(rxnList, {sampleStruct_LP.points, sampleStruct_M.points, sampleStruct_HP.points}, {model_LP, model_M, model_HP},[],[3,5]);
